---
title: Is AI Coding Modern Pair Programming?
date: 2026-06-27
description: AI Application Engineering with Pair Programming
---
...
1. First, render each page of the invoice PDF using some PDF rendering library. This is a deterministic (which doesn't depend on probabilistic deep neural network-kind models) part.  
2. Extract information from each image using some open-source transformer model. When there are multiple pages, again use the model to merge them into a single JSON text.  
3. Then we need to map this raw JSON text to a particular schema which is required by any bill-processing accounting tools (in my case I am using Zoho Books for this). This needs the cognitive ability of the model to map the data to the desired format. Sometimes fields might be in different names, so the model needs to figure out the mapping.  
4. Resolve IDs (identifiers that belong to Zoho Books like Vendor ID, Account ID, Tax ID, Currency ID, etc.) to send in the bill creation payload. It is done deterministically now. But there is a story behind it which I will explain later in the blog.  
5. Discard unused and unnecessary fields from the payload and send it to the server using the Zoho Books API.
...
1. Each time I would plan something to make the pipeline better.  
2. Ask the AI agent to create an implementation plan. Review the plan and refine it.  
3. Ask the AI agent to do the implementation, and review and make any necessary changes needed in the implementation by myself or by instructing the agent to do that.  
4. Test it and repeat this cycle until I am satisfied with the end result.
